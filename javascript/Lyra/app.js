vde.App.controller('DataCtrl', function($scope, $rootScope, $http, timeline, Vis, vg, d3) {
  ...
  $scope.loadValues = function() {
    ...
    if($scope.dMdl.from == 'url') {
        ...
        var dataToSend = JSON.stringify({ url: req.url });

        var json_returned = (function () {
            var temp = null;
            $.ajax({
                type: "GET",
                url: "ReportsData.svc/JSON/" + req.url,
                data: {},
                async: false,
        ...
        if (json_returned !== null)
        {
            $scope.dMdl.src.values = json_returned;
        }
        else
        {
            sweetAlert("Sorry", "File not found");
        }
    ...
    $timeout(function() {
      ...
      Vis.render().then(function() {
        ...
        if (vg.keys(Vis._rawData).length === 0) {
           Vis.data('PTSB holdings', 'data/ptsb_holdings.json', 'json');
        }
      });
    }, 500);
  };
...
vde.App.directive('vdeBinding', function($compile, $rootScope, $timeout, timeline, Vis, iVis) {
  return {
    ...
    templateUrl: 'templates/Lyra/inspectors/binding.html',
...
vde.App.directive('vdeCanDropField', function() {
  return {
    restrict: 'E',
    templateUrl: 'templates/Lyra/inspectors/can-drop-field.html',
...
vde.App.directive('vdeDataGrid', function () {
  return {
    ...
    templateUrl: 'templates/Lyra/inspectors/datasheet.html',
    controller: function($scope) {
...
vde.App.directive('vdeProperty', function($rootScope, timeline, Vis, iVis, vg) {
  return {
    ...
    templateUrl: function(tElement, tAttrs) {
      var tmpl = 'templates/Lyra/inspectors/property';
...
vde.App.directive('vdeScaleValues', function(Vis, vg) {
  return {
    ...
    templateUrl: 'templates/Lyra/inspectors/scale-values.html',
    controller: function($scope) {
